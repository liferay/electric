...
function api(project, options) {
	const apiEntitiesPath = project.apiEntitiesPath || 'electric-marble-components/lib/ElectricAPIEntities';
	...
	return through.obj(function(file, enc, cb) {
		...
		if (file.isBuffer()) {
			...
			addVinylFiles(stream, {
				apiEntitiesPath: apiEntitiesPath,
				name: 'index',
				namespace: getRandomString()
			}, layout, options.metalComponents);

			apiData.forEach(function(item) {
				item.apiEntitiesPath = apiEntitiesPath;
...
function addVinylFiles(stream, item, layout, metalComponents) {
	stream.push(
		creatVinylFile(
			apiSoywebTemplate({
				apiEntitiesPath: item.apiEntitiesPath,
				layout: layout,
