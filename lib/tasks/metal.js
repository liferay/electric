...
module.exports = function(options) {
	...
	gulp.task(taskPrefix + 'metal:prep:pages', function() {
		...
		var partials = _.map(globby.sync(path.join(TEMP_DIR, '+(components|layouts|partials)/**/*.soy')), function(filePath) {
	...
	gulp.task(taskPrefix + 'metal:render:globals', [taskPrefix + 'metal:render:soy'], function() {
		return gulp.src([path.join(TEMP_DIR, '**/*.js'), '!' + path.join(TEMP_DIR, '**/*.soy.js')])
			.pipe(bundle({
				dest: pathDest,
	...
	gulp.task(taskPrefix + 'metal:render:soy', function() {
		...
		return gulp.src(path.join(TEMP_DIR, '**/*.soy'))
			.pipe(compileSoy({
					outputDir: path.join(__dirname, '../../temp/metal'),
					soyDeps: soyDeps
				})
