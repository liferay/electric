...
module.exports = function(options) {
	...
	gulp.task(taskPrefix + 'metal:render:globals', [taskPrefix + 'metal:render:soy'], function() {
		var codeTabPath = path.join(require.resolve('electric-code-tabs/package.json'), '..');
		var codeTabFile = path.join(codeTabPath, 'src/*.js');

		return gulp.src([path.join(TEMP_DIR, '**/*.js'), codeTabFile])
			.pipe(buildGlobals({
	...
	gulp.task(taskPrefix + 'metal:render:soy', function() {
		...
		if (metalSoyPaths.length) {
			soyDeps = soyDeps.concat(metalSoyPaths);
		}
