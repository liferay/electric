...
module.exports = function(options) {
	...
	gulp.task(taskPrefix + 'soyweb:compile', function() {
		...
		var srcPath = [path.join(TEMP_DIR_SITE, '**/*.soy')];
	...
	function getPageContext(file, siteData) {
		var url = util.getPageURL(file.path, path.join(TEMP_DIR_SITE, 'pages'));
	...
	function getAPIContext(file, siteData, ref) {
		var apiData = require(path.join(process.cwd(), TEMP_DIR_SITE, 'pages/api', ref, 'API.json'));
