...
module.exports = function(options) {
	...
	gulp.task(taskPrefix + 'soyweb:prep', function() {
		...
		return gulp.src(path.join(pathSrc, '**/*.+(md|soy)'))
			...
			.pipe(inject(gulp.src(path.join(process.cwd(), pathDest, 'vendor/**/*'), {
				read: false
			}), {
				ignorePath: pathDest
			}))
			...
			.pipe(inject(gulp.src(path.join(__dirname, '../templates/codemirror.tpl')), {
				starttag: '<!-- inject:codemirror:js -->',
				transform: function(filePath, file) {
