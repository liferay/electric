...
let apiSoywebTemplate = _.template(
...
function api(project, options) {
	let cwd = process.cwd();
	let layout = options.apiConfig.layout;
...
function creatVinylFile(item, layout) {
	return new gutil.File({
		...
		contents: new Buffer(
			apiSoywebTemplate({
				layout: layout,
				namespace: _.camelCase(
					_.deburr(item.context.file) + '_' + item.name
				) + getRandomString()
			})
		),
		cwd: process.cwd(),
...
function resolveContext(item, project) {
	let filePath = item.context.file;
